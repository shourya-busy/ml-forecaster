"""Small LSTM forecaster (PyTorch).

Trains on normalized lag sequences and forecasts recursively. CUDA is
used automatically when FORECASTER_USE_CUDA=1 and a device is available.
"""
...
class LSTMForecaster(BaseForecaster):
    ...
    def fit(self, series: pd.Series) -> None:
        ...
        if len(series) < self.lags + 2:
            raise ValueError(f"lstm: need at least {self.lags + 2} points, got {len(series)}")
        ...
        class Net(nn.Module):
            def __init__(self, hs: int, nl: int):
                super().__init__()
                self.lstm = nn.LSTM(input_size=1, hidden_size=hs, num_layers=nl, batch_first=True)
        ...
        # Drop the live net; rebuild on predict() so artifact pickles small.
